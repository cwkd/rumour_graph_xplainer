import os
from Process.dataset import GraphDataset, BiGraphDataset, UdGraphDataset, BERTDataset
cwd=os.getcwd()
...
def loadTree(dataname):
    if 'Twitter' in dataname:
        ...
        treePath = os.path.join(cwd, 'data', 'data.TD_RvNN.vol_5000.txt')
...
def loadUdData(dataname, treeDic, fold_x_train, fold_x_test, droprate):
    data_path = os.path.join(cwd, 'data', f'{dataname}graph')
    print("loading train set", )
...
def loadBERTData(dataname, treeDic, fold_x_train, fold_x_test, TDdroprate, BUdroprate):
    data_path = os.path.join(cwd,'data', dataname + 'graph')
    print("loading train set", )
    traindata_list = BERTDataset(fold_x_train, treeDic, tddroprate=TDdroprate, budroprate=BUdroprate,
                                    data_path=data_path)
    print("train no:", len(traindata_list))
    print("loading test set", )
    testdata_list = BERTDataset(fold_x_test, treeDic, data_path=data_path)
    print("test no:", len(testdata_list))
    return traindata_list, testdata_list
