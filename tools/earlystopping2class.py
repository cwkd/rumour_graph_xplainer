...
class EarlyStopping:
    ...
    def __call__(self, val_loss, accs, acc1, acc2, pre1, pre2, rec1, rec2, F1, F2, model, modelname, datasetname,
                 checkpoint):
        ...
        if self.best_score is None:
            ...
            if self.counter >= self.patience:
                ...
                print("BEST LOSS:{:.4f}| Accuracy: {:.4f}|acc1: {:.4f}|acc2: {:.4f}|pre1: {:.4f}|pre2: {:.4f}"
                      ...
                      .format(-self.best_score, self.accs, self.acc1, self.acc2, self.pre1, self.pre2,
                              self.rec1, self.rec2, self.F1, self.F2))
                self.save_checkpoint(val_loss, model, modelname, datasetname)
        else:
            self.best_score = score
            self.accs = accs
    ...
    def save_checkpoint(self, val_loss, model, modelname, datasetname):
        ...
        save_path = f'best_{modelname}_{datasetname}_f{fold}_i{i}_e{epoch:05d}_l{loss:.5f}.pt'
